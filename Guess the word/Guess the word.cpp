...
void set(const char* str, char user_word[], char print[]) {
    for (int i = 0; str[i] != '\0'; i++) {
        if (str[i] == user_word[i]) {
            print[i] = str[i];
        }
    } 
}

void ptint(char print[]) {
    cout << "\n" << string(50, '-') << "\n";
...
bool is_win(const char* str, char user_word[]) {
    for (int i = 0; str[i] != '\0'; i++)
    {
        if (str[i] != user_word[i]) {
            return false;
...
void menu(const char* str) {
   ...
   do{
    system("cls");
    set(str, user_word, print);
    ptint(print);
    cout << "\nEnter word: ";
    gets_s(user_word);
   } while (!is_win(str, user_word));
   cout << "You win!🎉";
}
